...
public:
	...
	Str& operator+=(const Str& s) {
		...
		//새로운 value와 length값을 저장하기

		std::copy(value, s.value + s.length, new_value + new_length);
		value = new_value;
		length = new_length;
		return *this;
	}
...
Str operator+(const Str&, const char*);//q4
Str operator+(const char*, const Str&);//q11
//<,>,<=, >=, ==,!= 비교연산자 필요
bool operator<(std::ostream&, const Str&); //q3
bool operator>(std::ostream&, const Str&); //q3
bool operator<=(std::ostream&, const Str&); //q3
bool operator>=(std::ostream&, const Str&); //q3
bool operator==(std::ostream&, const Str&); //q4
bool operator!=(std::ostream&, const Str&); //q4
